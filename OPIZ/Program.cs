...
namespace OPIZ
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(string.Join("\n",     string.Join("\n", simpson.Ys.Select(x => $"{x.i:00}  {x.x:00.000}   {x.y:00.000} {simpson.f(x.x):00.000}"))));

            var square = new Integral((0, 1), (x) => (2 * x + Math.Exp(-(x * x))), 10);
            Console.WriteLine($"Формула прямоугольния: \n {square.SolveRect()}");
            Console.WriteLine("i\tx\ty\tf");
            Console.WriteLine(string.Join("\n", string.Join("\n", square.Ys.Select(x => $"{x.i:00}  {x.x:00.000}   {x.y:00.000} {square.f(x.x):00.000}"))));
        ...
        {
            Console.WriteLine("i\tx\ty\tf");
            Console.WriteLine(new Lb4(1, 0.1, (0, 1), (x, y) => (y + Math.Cos(x) / 3)).Task1());
